...
class PostListAdapterHelper(
    ...
    private val defaultLayoutParams: LinearLayout.LayoutParams = LinearLayout.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT, ViewGroup.LayoutParams.WRAP_CONTENT)
    ...
    private fun getMaxWidth(floor: String): Float {
        var maxWidth: Float = BaseApplication.ScreenInfo.EXACT_SCREEN_WIDTH.toFloat() - (24 * 2 + 38).dpToPx()
        if (pureRead || "1" == floor) {
            maxWidth = BaseApplication.ScreenInfo.EXACT_SCREEN_WIDTH.toFloat() - (16 * 2 + 4).dpToPx()
        }
        return maxWidth
    }

    private fun getLayoutParams(contentBean: ThreadContentBean.ContentBean, floor: String): LinearLayout.LayoutParams {
    ...
    private fun getLinkContent(oldContent: CharSequence, newContent: CharSequence?, url: String): CharSequence {
        val linkIconText = "[链接]"
        val s = " "
        ...
        spannableStringBuilder.append(linkIconText, MyImageSpan(context, bitmap), Spanned.SPAN_EXCLUSIVE_EXCLUSIVE)
        spannableStringBuilder.append(s)
        spannableStringBuilder.append(newContent)
        spannableStringBuilder.setSpan(MyURLSpan(context, url), start, end, Spanned.SPAN_EXCLUSIVE_EXCLUSIVE)
        return spannableStringBuilder
    }

    private fun appendLinkToLastTextView(views: List<View>, newContent: CharSequence?, url: String): Boolean {
        val lastView = views.lastOrNull()
        if (lastView is TextView) {
    ...
    private fun getUserContent(oldContent: CharSequence, newContent: CharSequence?, uid: String): CharSequence {
        val start = oldContent.length
        val end = start + (newContent ?: "").length
        val spannableStringBuilder = SpannableStringBuilder(oldContent)
        spannableStringBuilder.append(newContent)
        spannableStringBuilder.setSpan(MyUserSpan(context, uid), start, end, Spanned.SPAN_EXCLUSIVE_EXCLUSIVE)
        return spannableStringBuilder
    }

    private fun appendUserToLastTextView(views: List<View>, newContent: CharSequence?, uid: String): Boolean {
        val lastView = views.lastOrNull()
        if (lastView is TextView) {
    ...
    private fun getPhotoViewBeans(): List<PhotoViewBean> {
        ...
        for (key in photoViewBeansMap.keys) {
            if (photoViewBeansMap.get(key) != null) photoViewBeans.addAll(photoViewBeansMap[key]
                    ?: emptyList())
        }
    ...
    fun getContentViews(postListItemBean: PostListItemBean): List<View> {
        ...
        for (contentBean in postListItemBean.content!!) {
            when (contentBean.type) {
                ...
                "3" -> {
                    val url = ImageUtil.getUrl(context, true, contentBean.originSrc!!, contentBean.bigCdnSrc, contentBean.cdnSrcActive, contentBean.cdnSrc)
                ...
                "5" -> if (contentBean.src != null && contentBean.width != null && contentBean.height != null) {
                    if (contentBean.link != null) {
                        ...
                        videoPlayerStandard.layoutParams = getLayoutParams(contentBean, postListItemBean.floor!!)
                        videoPlayerStandard.id = R.id.video_player
                        ImageUtil.load(videoPlayerStandard.posterImageView, ImageUtil.LOAD_TYPE_SMALL_PIC, contentBean.src, true)
                        views.add(videoPlayerStandard)
                    } else {
                        val videoImageView = MyImageView(context)
                        videoImageView.layoutParams = getLayoutParams(contentBean, postListItemBean.floor!!)
                        videoImageView.scaleType = ImageView.ScaleType.CENTER_INSIDE
                        ImageUtil.load(videoImageView, ImageUtil.LOAD_TYPE_SMALL_PIC, contentBean.src, true)
                    ...
                    if (appendLinkToLastTextView(views, "[视频] " + contentBean.text, contentBean.text!!)) {
                        val textView: TextView = createTextView()
                        textView.layoutParams = defaultLayoutParams
                        setText(textView, getLinkContent("[视频] " + contentBean.text, contentBean.text!!))
                        views.add(textView)
                    }
                }
                "10" -> {
                    val voiceUrl = "http://c.tieba.baidu.com/c/p/voice?voice_md5=" + contentBean.voiceMD5 + "&play_from=pb_voice_play"
                    val voicePlayerView = VoicePlayerView(context)
                    voicePlayerView.layoutParams = LinearLayout.LayoutParams(ViewGroup.LayoutParams.WRAP_CONTENT, ViewGroup.LayoutParams.WRAP_CONTENT)
                ...
                "20" -> {
                    ...
                    memeImageView.layoutParams = getLayoutParams(contentBean, postListItemBean.floor!!)
                    ...
                    ImageUtil.initImageView(memeImageView, PhotoViewBean(contentBean.src, contentBean.src, false))
